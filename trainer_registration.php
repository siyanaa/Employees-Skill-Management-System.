...
if(!$conn){
    ...
    if(isset($_POST['submit'])){
        ...
        $hash = md5($password);
        if(empty($id) || empty($fullname) || empty($phone) ||  empty($email) ||  empty($experience) ||   empty($gender) || empty($course) || empty($hourly_charge) ||  empty($username) || empty($password) || empty($confirm)){
    ...
    if(($confirm)!=($password)){
        ...
        if(!$conn){
            die("Connection Failed:");
             ...
             $sql2="INSERT INTO trainer(
            trainer_id,fullname,phone,email,experience,gender,course,hourly_charge,username,password,confirm_password)
            VALUES('$id','$fullname','$phone','$email','$experience','$gender','$course','$hourly_charge','$username','$hash','$hash')";
...
<center>
<i class="fa fa-users"> TRAINER REGISTRATION!</h1></i>
</center>
  ...
  <div class="col-md-12">
      ...
      <form  method="post">
        <div class="row centered-form">
          <div class="col-50">
          <?php
            $sql1="SELECT trainer_id from trainer ORDER BY trainer_id DESC LIMIT 1;";
            $result1= mysqli_query($conn,$sql1);
            $row1= mysqli_fetch_assoc($result1);

            if(mysqli_num_rows($result1)>0){
                $val=$row1['trainer_id'] + 1;
            }
            else{
                $val = 1;
            }
		  ...
		  <div class="form-group">
			...
			<input type="text" id="trainer-id" name="id" required pattern="[0-9]+"  readonly='readonly' required class="form-control input-sm" value="<?php echo $val?>"?>
    ...
    <div class="radio-inline">
        <label class="control-label text-primary">
            <input type="radio" name="gender" class="radio" value="Others" required class="form-control input-sm"> Others
				...
				<label class="control-label text-primary"><i class="fa fa-book"></i> Courses </label>
            <input type="text" name="course" placeholder="" pattern="^[A-Za-z]+(\s*,?\s*[A-Za-z]+)*$" required class="form-control input-sm">
			<div class="col-50">

			<label class="control-label text-primary"><i class="fa fa-dollar"></i> Rate Per Class</label>
            <input type="text" name="hourly_charge" placeholder="" pattern="[0-9]+" required class="form-control input-sm">
