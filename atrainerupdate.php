...
if(isset($_POST['submit'])){
  ...
  $sql="UPDATE trainer set fullname='$fullname',phone='$phone',email='$email',experience='$experience',gender='$gender',course='$course',hourly_charge='$hourly_charge' where trainer_id='$id'";
      ...
      <form  method="post">
        <div class="row">
          <div class="col-50">
            ...
            <input type="text" name="id" value="<?php echo $data['trainer_id']?>" readonly='readonly' required class="form-control input-sm">
            ...
            <input type="text" name="course" value="<?php echo $data['course']?>" pattern="^[A-Za-z]+(\s*,?\s*[A-Za-z]+)*$" required class="form-control input-sm"  >
            <label class="control-label text-primary"><i class="fa fa-dollar"></i> Rate Per Class</label>
            <input type="text" name="hourly_charge" value="<?php echo $data['hourly_charge']?>" pattern="[0-9]+" required class="form-control input-sm"  >

          </div>

          
        </div>
        <button type="submit" name="submit" class="btn btn-primary" onclick="return confirm('Are you sure you wanna edit?')">Edit Trainer</button><br><br>
      </form>
    </div>
  </div>
 </div>
